...
import { notifyGoogleAdsWhatsAppClick, notifyAffiliateWhatsAppClick } from '@/lib/tracking/whatsapp-alert'
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Slack ping when a Google Ads visitor contacts us on WhatsApp, or when a
    // partner-referred visitor does. Runs before the insert so each "first tap
    // of the session" check is accurate.
    if (event_name === 'whatsapp_click') {
      await notifyGoogleAdsWhatsAppClick(supabase, session_id, metadata)
      await notifyAffiliateWhatsAppClick(supabase, session_id, metadata)
    }
